...
class ObjectTypeHierarchy:
    ...
    def height(self, lev: int) -> int:
        if self.cover_up:
            ...
            while q:
                u = q.popleft()
                for v in self.cover_up.get(u, []):
                    if v not in dist:
                        dist[v] = dist[u] + 1
                        q.append(v)
            return dist.get(lev, lev)
        k = self.k
        if k <= 1:
            return lev
        if lev == 0:
...
class LevelConfiguration:
    ...
    def __init__(self, config: Dict[str, int], hierarchies: Dict[str, ObjectTypeHierarchy], obj_types: Dict[str, str]) -> None:
        self.config = config
...
def compute_footprint(beh: BehaviorGraph) -> BehaviorFootprint:
    ...
    obj_pairs = frozenset((pair for pairs in beh.node_objects.values() for pair in pairs))
    edge_ms = frozenset(((str(beh.edge_label[ae]), beh.edge_kappa[ae][0], beh.edge_kappa[ae][1]) for ae in beh.abstract_edges))
    ...
    for ae in beh.abstract_edges:
        ...
        out_deg[s] += 1
        in_deg[t] += 1
    deg_seq = tuple(sorted(((in_deg.get(n, 0), out_deg.get(n, 0)) for n in beh.abstract_nodes)))
    return BehaviorFootprint(n_nodes=len(beh.abstract_nodes), n_edges=len(beh.abstract_edges), act_multiset=act_ms, obj_multiset=obj_pairs, edge_multiset=edge_ms, degree_seq=deg_seq)
...
def _index_ocel(ocel: OCEL):
    ...
    o2tr = {oid: sorted(grp[EV_ID].tolist(), key=lambda e: ts.get(e)) for oid, grp in ocel.relations.groupby(OBJ_ID)}
    einfo = {e: {'activity': act[e], 'timestamp': ts[e], 'objects': e2o[e]} for e in ocel.events[EV_ID]}
...
def compute_behavior(lg: OCELLevelGraph) -> BehaviorGraph:
    ...
    if n == 0:
        empty: Dict = {}
        return BehaviorGraph(set(), set(), empty, empty, empty, empty, empty, empty, empty)
            ...
            if n_perturbed < len(members):
                ...
                for i in members:
                    if i not in perturbed:
                        ...
                        break
                if rep != -1:
                    ins_rep = frozenset(((edge_lbl_l[k], part_l[edge_src_l[k]]) for k in in_e_l[rep]))
                    outs_rep = frozenset(((edge_lbl_l[k], part_l[edge_tgt_l[k]]) for k in out_e_l[rep]))
                    stable_list = [i for i in members if i not in perturbed]
                    sig_groups[ins_rep, outs_rep] = stable_list
            for i in perturbed:
                ins = frozenset(((edge_lbl_l[k], part_l[edge_src_l[k]]) for k in in_e_l[i]))
                outs = frozenset(((edge_lbl_l[k], part_l[edge_tgt_l[k]]) for k in out_e_l[i]))
                sig = (ins, outs)
    ...
    for (sc, tc, lbl), ai in ak2i.items():
        ...
        ee[ai] = (sc, tc)
    return BehaviorGraph(set(part), set(ak2i.values()), nact, nobj, el, ek, ee, alpha, beta)

def _beh_to_nx(beh: BehaviorGraph) -> nx.DiGraph:
    G: nx.DiGraph = nx.DiGraph()
    for n in beh.abstract_nodes:
        G.add_node(n, act=beh.node_activity[n], obj=frozenset(beh.node_objects[n]))
...
def _are_isomorphic_nx(g1: 'nx.DiGraph', g2: 'nx.DiGraph') -> bool:
    ...
    if g1.number_of_edges() != g2.number_of_edges():
        return False
    return nx.is_isomorphic(g1, g2, node_match=_iso_node_match, edge_match=_iso_edge_match)
