...
def parse_timestamp(value):
    ...
    raise ValueError(f"Timestamp format not recognized: {value}")

def to_string(x):
    if isinstance(x, tuple):
        return "__".join(
            str(v).replace(" ", "_").replace("-", "_").replace("/", "_")
            for v in x
        )
...
def mlpaDiscovery(ocel, tau=0.9):
    ...
    import pprint
    pprint.pprint(resulting_process_view_with_events, sort_dicts=True)
    return levels_dict, resulting_process_view, resulting_process_view_with_events